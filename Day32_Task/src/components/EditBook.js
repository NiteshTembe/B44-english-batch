...
export function EditBook() {
  ...
  if(book){
    return <EditBookDetails book={book}/>
  }
  else return null
...
function EditBookDetails({book}){
  ...
  const formik = useFormik({
    ...
    onSubmit: (bookData) => {
      //console.log("onSubmit bookData Data", bookData);
      EditBookData(bookData);
    },
  });

  const EditBookData = async (bookData) => {
    ...
    if (res.status === 200) {
      setOpen({ type: "success", msg: `Book Edited Successfully !` });
    } else {
