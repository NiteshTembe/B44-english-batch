            ...
            <nav className="main-menu">
                <ul>
                    <li className="logo-fixed">
                        <div className="row d-flex align-items-center" style={{ width: "fit-content", marginLeft: '8px' }}>
                            <div className='sidebar-logo p-0'>
                                <img
                                    ...
                                    alt="" />
                                <h6 className="mx-3 d-inline-block mt-3 font-weight-bold">{auth.user_role}</h6>
                            </div>
                        </div></li>
                    <SidebarListItem toLink={"/dashboard"} listTitle={"Dashboard"} iconElement={<DashboardIcon />}/>
                    <SidebarListItem toLink={"/queries"} listTitle={"Queries"} iconElement={<QuestionAnswerIcon/>}/>
                </ul></nav></div>
    );
};

const SidebarListItem = ({listTitle, iconElement, toLink}) => {
    const navigate = useNavigate()    
    const location = useLocation();
    const isActive = location.pathname === toLink ? "list-scroll py-3 active-area active-left-bar" : "list-scroll py-3 color-area";
    return (
        <div className={isActive}>
            <li onClick={()=>(navigate(toLink))}>
                <span className="mx-4">
                    <div className="span-area d-flex justify-content-between align-items-center">
                        <div color="" className="sc-gueYoa kYDnie">{iconElement}</div>
                        <div className="mx-4">{listTitle}</div>
                    </div>
                </span>
            </li>
        </div>
